...
const Vehicles = () => {
  ...
  return (
    <div id="vehicles" className="py-24 relative">
          ...
          {displayedVehicles.map((vehicle) => (
            <div key={vehicle.id} className="group bg-white rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col">
              <div className="relative h-60 overflow-hidden">
                ...
                />
                <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-md px-4 py-1 rounded-full text-primary font-black text-xs shadow-xl">
                  {vehicle.type}
                </div>
              </div>

              <div className="p-7 flex-1 flex flex-col">
                <h3 className="text-2xl font-black text-primary mb-4 group-hover:text-secondary transition-colors leading-tight">{vehicle.name}</h3>
                ...
                <div className="flex items-center justify-between mt-auto">
                  <div className="flex flex-col">
                    <span className="text-[9px] uppercase font-bold text-gray-400 tracking-tighter">Starting from</span>
                    <span className="text-xl font-black text-primary">{vehicle.price}</span>
                  </div>
                  <button className="bg-primary text-white w-12 h-12 rounded-xl flex items-center justify-center hover:bg-secondary transition-all hover:scale-110 shadow-xl group/btn">
